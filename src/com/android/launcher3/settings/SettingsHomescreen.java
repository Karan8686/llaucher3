...
public class SettingsHomescreen extends CollapsingToolbarBaseActivity
    ...
    public static class HomescreenSettingsFragment extends SettingsBasePreferenceFragment implements
        ...
        public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
            ...
            mHotseatQsbStrokeWidthPref = screen.findPreference(LauncherPrefs.HOTSEAT_QSB_STROKE_WIDTH.getSharedPrefKey());
        ...
        private void updateIsGoogleAppEnabled() {
            boolean isGSAEnabled = Utilities.isGSAEnabled(getContext());
            
            String notSystemSummary = getString(R.string.google_app_not_system_summary);
            if (mShowGoogleAppPref != null) {
                mShowGoogleAppPref.setEnabled(isGSAEnabled);
                if (!isGSAEnabled) {
                    mShowGoogleAppPref.setSummary(notSystemSummary);
                } else {
                    mShowGoogleAppPref.setSummary(R.string.pref_show_google_now_summary);
                }
            }
            if (mShowGoogleBarPref != null) {
                mShowGoogleBarPref.setEnabled(isGSAEnabled);
                if (!isGSAEnabled) {
                    mShowGoogleBarPref.setSummary(notSystemSummary);
                } else {
                    mShowGoogleBarPref.setSummary(R.string.dock_search_summary);
                }
            }
            if (mHotseatQsbOpacityPref != null) {
                mHotseatQsbOpacityPref.setEnabled(isGSAEnabled);
            }
            if (mHotseatQsbStrokeWidthPref != null) {
                mHotseatQsbStrokeWidthPref.setEnabled(isGSAEnabled);
            }
            if (mDockMusicSearchPref != null) {
                mDockMusicSearchPref.setEnabled(isGSAEnabled);
            }
            if (mDockThemePref != null) {
                mDockThemePref.setEnabled(isGSAEnabled);
            }
            if (mSearchRadiusSizePref != null) {
                mSearchRadiusSizePref.setEnabled(isGSAEnabled);
            }
        }
